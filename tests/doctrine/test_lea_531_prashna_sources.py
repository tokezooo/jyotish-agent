...
def test_case_metadata_is_traceable_and_high_stakes_cases_are_excluded() -> None:
    ...
    case = cases.cases[0]
    assert case.question_at == "1983-03-31T18:30:00+05:30"
    assert case.place_label == "Bangalore, India"
    assert case.source_pdf_pages == (22, 23)
    assert case.judgment == "negative"
    assert case.observed_outcome == "alliance_broke_off"
    assert case.admissibility == "excluded_high_stakes"
    assert cases.verified_outcome_count == 1


def test_verified_bytes_cover_sources_but_missing_cases_keep_release_closed(
    tmp_path: Path,
) -> None:
    ...
    assert report.verified_published_case_count == 1
    assert report.missing_published_case_count == 1
    assert "published_cases" in report.missing_requirements
